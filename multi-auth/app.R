# Multi-Source Dashboard - Find by Integration Name
# ===================================================
# Uses get_associations() + Filter by name to discover
# both Snowflake and Databricks integration GUIDs.
#
# Required env vars:
...
ui <- page_sidebar(
  title = "Multi-Source Analytics (find by name)",
  fillable = FALSE,
  sidebar = sidebar(
    width = 260,
    h3("Multi-Source Dashboard"),
    p(
      "Snowflake + Databricks via find by name (connectapi)",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$load_data, {
    ...
    if (length(sf_assoc) == 0 || length(db_assoc) == 0) {
      showNotification(
        paste0("Missing: snowflake(", sf_name, ")=", length(sf_assoc) > 0,
               ", databricks(", db_name, ")=", length(db_assoc) > 0),
        type = "error"
      )
      return()
    }
